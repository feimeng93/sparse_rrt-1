...
double l_obs_t::distance(const double* point1, const double* point2, unsigned int state_dimension)
{
	double sum = 0;
...
bool l_obs_t::valid_state()
{
	bool obstacle_collision = false;
	//any obstacles need to be checked here
	for(unsigned i=0;i<obs_list.size() && !obstacle_collision;i++)
	{
		if(	temp_state[0]>=obs_list[i][0] && 
			temp_state[0]<=obs_list[i][2] && 
			temp_state[1]>=obs_list[i][5] && 
			temp_state[1]<=obs_list[i][1])
		{
...
void l_obs_t::update_derivative(const double* control)
{
...
std::tuple<double, double> l_obs_t::visualize_point(const double* state, unsigned int state_dimension) const
{
...
std::string l_obs_t::visualize_obstacles(int image_width, int image_height) const
{
	...
	for(unsigned i=0;i<obs_list.size();i++)
	{
		...
		doc<<svg::Rectangle(svg::Point(x*dims.width, y*dims.height),
							(obs_list[i][2]-obs_list[i][0])/(MAX_X-MIN_X) * dims.width,
							(obs_list[i][1]-obs_list[i][5])/(MAX_Y-MIN_Y) * dims.height,
							svg::Color::Red);
	}
    return doc.toString();
}

std::vector<std::pair<double, double> > l_obs_t::get_state_bounds() const {
	return {
			{MIN_X,MAX_X},
			{MIN_Y,MAX_Y}
	};
...
std::vector<std::pair<double, double> > l_obs_t::get_control_bounds() const {
	return {
			{MIN_V, MAX_V}
	};
}

std::vector<bool> l_obs_t::is_circular_topology() const{
    return {
            false,
            false
    };
