...
#ifndef SPARSE_PNL_OBS_HPP
#define SPARSE_PNL_OBS_HPP
 ...
 */
class pnl_obs_t : public enhanced_system_t
{
public:
	pnl_obs_t(std::vector<std::vector<double>>& _obs_list, double width)
	{
		state_dimension = 3;
		control_dimension = 1;
		validity = true;
	...
	virtual ~pnl_obs_t(){ delete[] temp_state; delete[] deriv; obs_list.clear();}
	 ...
	 */
	std::tuple<double, double, double> visualize_3Dpoint(const double* state, unsigned int state_dimension) const override;
